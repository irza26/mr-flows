...
WRF_CACHE = {"data": None, "last_fetch": 0}


def get_cached_wrf():
    now = time.time()
    if WRF_CACHE["data"] is not None and (now - WRF_CACHE["last_fetch"] < 300):
    ...
    try:
        ...
        print("WRF ERROR:", e)
        # Kembalikan cache lama kalau ada, atau dict kosong
        return WRF_CACHE["data"] or {}
...
def ch_dashboard(station: str):
    with get_db() as conn:
        ...
        try:
            ...
            cur.execute("""
                SELECT MAX(datetime) FROM rainfall WHERE station = %s
            """, (station,))
            ...
            cur.execute("""
                SELECT rainfall FROM rainfall WHERE station = %s ORDER BY datetime DESC LIMIT 1
            """, (station,))
            current = cur.fetchone()[0] or 0

            cur.execute("""
                SELECT COALESCE(SUM(rainfall), 0) FROM rainfall
                WHERE station = %s AND datetime > %s - INTERVAL '1 hour' AND datetime <= %s
            ...
            chart = [
                {"time": r[0].strftime("%Y-%m-%d %H:%M"), "rain_fall": round(r[1], 2)}
                for r in cur.fetchall()
            ]

            wrf_data = get_cached_wrf()
            station_series = wrf_data.get(station, [])
            prediction_value = station_series[0]["rain_mm"] if station_series else 0
            prediction_chart = [
                {"time": t["time"], "rain": round(t["rain_mm"], 2)}
...
def ch_sidebar():
    with get_db() as conn:
        ...
        try:
            cur.execute("""
                WITH latest AS (
                    SELECT station, MAX(datetime) AS last_time FROM rainfall GROUP BY station
                )
                SELECT r.station, COALESCE(SUM(r.rainfall), 0) AS rain_per_hour, l.last_time
                FROM rainfall r
                JOIN latest l ON r.station = l.station
                WHERE r.datetime > l.last_time - INTERVAL '1 hour' AND r.datetime <= l.last_time
                GROUP BY r.station, l.last_time ORDER BY r.station
            """)
            rows = cur.fetchall()
            ...
            return [
                {
                    ...
                    "prediction": round(wrf_data.get(r[0], [{}])[0].get("rain_mm", 0) if wrf_data.get(r[0]) else 0, 2),
                }
                for r in rows
            ]
